...
def exists(req, resp, resource, params):
    if req.get_json('offering_id') == '__all__':  # Denotes that we need all the subjects.
        return

    try:
        db.Session.query(SubjectOffering).filter_by(subject_id=int(req.get_json('offering_id'))).one()
...
def not_exists(req, resp, resource, params):
    try:
        db.Session.query(SubjectOffering).filter_by(subject_id=req.get_json('subject_id')).one()
